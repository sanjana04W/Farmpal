// Feed Consumption Screen for FarmPal
...
import { addFeedRecord, getFeedByMonth } from '../services/feedService';
import { getCurrentDateComponents, getDaysInMonth } from '../utils/dateUtils';

const { width } = Dimensions.get('window');

const FeedConsumptionScreen = ({ navigation }) => {
    ...
    const loadData = async () => {
        try {
            setLoading(true);
            const data = await getFeedByMonth(selectedMonth, selectedYear);
            setFeedData(data);
        } catch (error) {
    ...
    const handleAddRecord = async () => {
        ...
        if (isNaN(amount) || amount <= 0) {
            Alert.alert('Invalid Input', 'Please enter a valid amount in Kg');
            return;
        }

        try {
            setSubmitting(true);
            await addFeedRecord(amount);
            setInputValue('');
            await loadData();
    ...
    const prepareChartData = () => {
        ...
        for (let i = 0; i < daysInMonth; i++) {
            labels.push(`${i + 1}`);
            const record = feedData.find(r => r.day === i + 1);
            if (record) {
                chartData[i] = record.amount;
            }
        }
    ...
    const totalFeed = feedData.reduce((sum, record) => sum + record.amount, 0);
                    ...
                    {loading ? (
                        ...
                        <>
                            ...
                            <View style={styles.totalContainer}>
                                ...
                                <Text style={styles.totalValue}>{totalFeed.toFixed(1)} Kg</Text>
                            </View>
                            ...
                            <View style={styles.tableSection}>
                                ...
                                <DataTable data={feedData} type="feed" />
                            </View>
                        </>
                    )}
                </ScrollView>
